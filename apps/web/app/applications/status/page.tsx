...
export default function ApplicationStatus(){
 ...
 async function lookup(e:any){e.preventDefault();setError('');setData(undefined);try{const r=await fetch(`${process.env.NEXT_PUBLIC_API_URL||'http://localhost:8000/api/v1'}/cooperatives/applications/${encodeURIComponent(reference.trim())}`);if(!r.ok)throw new Error('Application not found');setData(await r.json())}catch(e:any){setError(e.message)}}
